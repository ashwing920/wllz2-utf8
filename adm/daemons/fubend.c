...
static mapping fuben_private_list = ([]);
static mapping fuben_private_name = ([]);
static mapping fuben_private_npc  = ([]);
static mapping fuben_private_all  = ([]);
...
int enter_fuben_private(object me, string file)
{
        ...
        if( !undefinedp(fuben_private_list[file]) ) {
                ...
                owner = me->query("id");
                startroom = load_object("/p/"+me->query("id")+file);
                if( startroom ) {           
                        ...
                        me->move(startroom);  
                        CHANNEL_D->do_channel(this_object(),"sys",me->query("name")+"("+owner+")"+"进入了副本"+fbname+"。");                    
...
protected void load_all_fuben()
{
        ...
        for( i=0;i<sizeof(dir);i++ ) {
                if( file_size(FUBEN_MAIN_DIR+"/"+dir[i]) == -2 ) {
						...
						//log_file("加载副本:"+dir[i]+"\n");
						//efun::write_file(LOG_DIR + "log", "加载副本:"+dir[i]+"\n");
                }
        }
        //load_fuben_private();
}
...
public object *query_fuben_players(string fbname, string owner)
{
        ...
        for( i=0;i<sz;i++ ) {
				...
				//CHANNEL_D->do_channel(this_object(),"sys","副本player:"+file);
