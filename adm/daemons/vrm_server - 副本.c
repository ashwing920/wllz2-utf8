...
private int random_rate = 20;		// 迷宫房间内出现NPC的随机率
private int char_num = 1;		// 迷宫出现NPC个数
...
private void paint_vrm_map()
{
	...
	map_file = sprintf( "%*.map",base_name(this_object()));
	write_file("/doc"+ map_file,output,1);
}
...
nomask object query_maze_room(string str)
{
	...
	if(str == "entry")	// 迷宫入口房间
	{
		...
		{
			ob->set("objects",([
	maze_npcs[random(sizeof(maze_npcs))] : random(char_num),
]));
			ob->setup();
		}
		return ob;
