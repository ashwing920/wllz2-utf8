...
mapping weapon_actions = ([
	...
	"hook":([
		"action":		"$N举起$w，眼明手快，猛的对$n的$l划了过去。",
		"damage_type":	"割伤",
                "parry":                20,
                "dodge":		20,
	]),
...
void bash_weapon(object me, object victim, object weapon, int damage)
{
        ...
        &&  ob = victim->query_temp("weapon") ) {
                ...
                if( wap > 2 * wdp ) {
                        ...
                        if(ob->query("ownermake")
                        && victim->query("weapon/lv") > 0){
                                victim->add("weapon/lv",-1);
                                tell_object(victim,HIW"你的"+ victim->query("weapon/name")+ HIW"伤害度降低了！\n"NOR);
                                }
                        }else{
                        ...
                        message_vision(HIY+"$N的" + weapon->name() + HIY+"和$n的" + ob->name()+ HIY+"相击，冒出点点的火星。\n" + NOR, me, victim);
