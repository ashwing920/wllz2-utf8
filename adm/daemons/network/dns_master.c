...
static mapping my_muds;
// the my_muds define for query my ip
	...
	i = sizeof( list );
        while( i-- ) {
                ...
                if( bootsrv[0] == addr && bootsrv[1] == args["PORTUDP"] )
                        ...
                        && args["_dns_no_contact"] < 1)
                        || args["MUDLIB"] == MUDLIB_NAME ) {
                                flag = 1;
                        }
        }
	if( flag ) {
...
string start_message()
{
	return sprintf( "||MUDNAME:%s||NAME:%s||VERSION:%s||DRIVER:%s||MUDLIB:%s"
		...
		code_name(),Mud_name(),MUDLIB_VERSION, __VERSION__, MUDLIB_NAME,
...
string my_ip() {
	...
	if(stringp(value = my_muds[query_mud_name()]))
...
void create()
{
	...
	// set up our own info
	this_host = ([
		"MUDNAME"       : code_name(),     // add by ken@XAJH 1997-10-18
		"NAME"          : Mud_name(),
		"ALIAS"         : Mud_name(),
