...
int is_apprentice_of(object ob)
{
        ...
        if( ob->id(family["master_id"])
        &&      family["master_name"] == (string)ob->query("name") )
                return 1;

        return 0;
}
int is_player_apprentice_of(object ob)
{
        mapping faction;
        if( !mapp(faction = query("faction")) ) return 0;
        if( faction["name"] != (string)ob->query("faction/name") ) return 0;
        if( !ob->query("faction/head") ) return 0;
        return 1;
}
...
int recruit_apprentice(object ob)
{
        ...
        if( my_family["family_name"] == "慕容世家"
        || my_family["family_name"] == "大理段家"
        || my_family["family_name"] == "白驼山派"
        || my_family["family_name"] == "灵鹫宫"
        || my_family["family_name"] == "唐门"){
                ob->assign_apprentice("传人", 0);
        }else{
