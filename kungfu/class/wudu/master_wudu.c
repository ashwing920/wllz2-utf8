...
#endif
#define FAMILY_MASTER   NPCDATA + "wudu";
...
void create()
{
	seteuid(getuid());
	if (!restore()) {
		set_name(npc_name("男性"),({ "master_wudu","master",}));
		set("family/family_name","五毒神教");
		set("gender", "男性" );
		set("age", 40);
...
int do_copy(object me, object ob)
{
	...
	me->set("short", me->query("title") + " " + me->query("name") + "[Master_wudu]");
	me->delete("title");
