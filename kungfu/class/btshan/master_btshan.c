...
#endif
#define FAMILY_MASTER   NPCDATA + "btshan";
...
void create()
{
	seteuid(getuid());
	if (!restore()) {
		set_name(npc_name("男性"),({ "master_btshan","master",}));
...
int accept_fight(object ob)
{
	...
	if( objectp(weapon = ob->query_temp("weapon")))
		return notify_fail("挑战少庄主必须空手！难道你想杀人不成！？\n");

	if( me->is_fighting() ) 
		return notify_fail("已经有人正在挑战少庄主了！\n");
...
int checking(object me, object ob)
{
	...
	if (( (int)me->query("qi")*100 / my_max_qi) <= 50 ) {
		command("say 果然厉害，恭喜你成为当今少庄主！\n");
		command("chat 哈哈哈，到底是长江後浪推前浪，一代新人换旧人！\n");
		command("chat 恭喜" + ob->query("name") + "成为"+ me->query("family/family_name")+"第" + chinese_number(me->query("generation")+1) + "代少庄主 ！\n");
...
int do_copy(object me, object ob)
{
	...
	me->set("title", ob->query("family/family_name")+"第" + chinese_number(me->query("generation")) + "代少庄主");
	me->set("short", me->query("title") + " " + me->query("name") + "[" + capitalize(ob->query("id")) + "]");
	ob->set("title", me->query("family/family_name")+"第" + chinese_number(me->query("generation")) + "代少庄主"NOR);
	
	ob->set_temp("apply/short", ({me->query("short")}));
	
	me->set("short", me->query("title") + " " + me->query("name") + "[Master_btshan]");
	me->delete("title");
