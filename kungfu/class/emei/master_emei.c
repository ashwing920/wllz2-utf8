...
#endif
#define FAMILY_MASTER   NPCDATA + "emei";
...
void create()
{
	seteuid(getuid());
	if (!restore()) {
		set_name(npc_name("女性"),({ "master_emei","master",}));
...
int accept_fight(object ob)
{
	...
	if( wizardp(ob))
		return notify_fail("你是巫师，不能挑战掌门师太。\n");

	if( ob->query("betrayer"))
		return notify_fail(me->name()+"说道：你这背信弃义的家伙，没资格向我挑战。\n");

	if( ob->query("combat_exp") < me->query("combat_exp") ) 
		return notify_fail("就凭你这微末得道行就想挑战掌门师太？\n");

	if( objectp(weapon = ob->query_temp("weapon")))
		return notify_fail("挑战掌门师太必须空手！难道你想杀人不成！？\n");

	if( me->is_fighting() ) 
		return notify_fail("已经有人正在挑战掌门师太了！\n");
...
int checking(object me, object ob)
{
	...
	if (( (int)me->query("qi")*100 / my_max_qi) <= 50 ) {
		command("say 果然厉害，恭喜你成为当今掌门师太！\n");
		command("chat 哈哈哈，到底是长江後浪推前浪，一代新人换旧人！\n");
		command("chat 恭喜" + ob->query("name") + "成为"+ me->query("family/family_name")+"第" + chinese_number(me->query("generation")+1) + "代掌门师太 ！\n");
...
int do_copy(object me, object ob)
{
	...
	me->set("title", ob->query("family/family_name")+"第" + chinese_number(me->query("generation")) + "代掌门师太");
	me->set("short", me->query("title") + " " + me->query("name") + "[" + capitalize(ob->query("id")) + "]");
	ob->set("title", me->query("family/family_name")+"第" + chinese_number(me->query("generation")) + "代掌门师太"NOR);
	
	ob->set_temp("apply/short", ({me->query("short")}));
	
	me->set("short", me->query("title") + " " + me->query("name") + "[Master_emei]");
	me->delete("title");
