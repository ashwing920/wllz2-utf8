...
void create()
{
	...
	set("class", "bonze");
	set("long",@LONG
他的面容奇特之极，左边的一半脸色红润，皮光肉滑，有如婴儿，右边的
一半却如枯骨，除了一张焦黄的面皮之外全无肌肉，骨头突了出来，宛然
便是半个骷髅骨头。这是他修习枯荣禅功所致。
LONG
);
	set("age", 100);
	set("shen_type", 1);
...
int ask_me()
{
	if ((int)this_player()->query_temp("marks/find_temp")&&this_player()->query_temp("marks/ask_temp3"))
		{
		say("枯荣看了" + this_player()->name() + "一眼，说道：\n""这位施主果然聪明，施主若能胜过老衲就可得到六脉神剑谱。\n");
		return 1;
	}
}
...
int accept_fight(object ob)
{
	...
	&& !ob->query_temp("fight_with_me") ) {
		say(ob->name() + "对枯荣说：好吧，那我就同你切搓几招吧，点到为止。\n");
		ob->set_temp("fight_with_me",1);
...
int checking(object me, object ob)
{
	...
	if(( (int)me->query("qi")*100 / my_max_qi) <= 50 ) {
		say("枯荣说：施主身手果然不凡，老衲佩服。六脉神剑谱是没有的了，这本一阳指诀你先拿去吧。\n");
		obj= new("/clone/book/sun_book");
		obj->move(ob);
		message("channel:rumor", HIM"【武林传奇】某人："+this_player()->query("name")+"获得了一阳指诀啦。\n"NOR, users());
		message_vision("$N交给$n一本书。\n", me, ob);
		return 1;
	}
	if (( (int)ob->query("qi")*100 / his_max_qi) < 50 ) {
		say("枯荣双手合十道：尘缘一切自心起，施主请回吧。\n");
...
string ask_six()
{
	...
	if (sizeof(skl) <= 1)
		return RANK_D->query_respect(fighter) + "功力不够，不够资格闯六脉剑阵。";
	sname  = keys(skl);
	for(i=0; i<sizeof(skl); i++) {
		if ((skl[sname[i]] < 80) || (sizeof(skl) == 0))
		return RANK_D->query_respect(fighter) + "，你这点本事也想闯六脉剑阵。";
	}
	if ( me->query("assigned_fighter") )
		return RANK_D->query_respect(fighter) + "，今日已有人挑战六脉剑阵，你过一段时间再来吧。";
	
	if(time() - me->query("fighttime") < 500)
		return RANK_D->query_respect(fighter) + "，今日已有人挑战六脉剑阵，你过一段时间再来吧。";
...
int preparing(object me, object fighter)
{
	...
	if(base_name(environment(this_object())) == "/d/tianlongsi/banruotai"){
		message("vision", HIY "\n枯荣大师大声宣布：六脉剑阵合围！\n" NOR, room);
		message("vision", "\n只听四周声声佛号，天龙寺高僧们个个神情肃杀，慢慢地从摩尼堂中鱼贯步出，围住了通路。\n\n", room);
		room->delete("exits");
...
int do_recruit(object me, object fighter)
{
	...
	else
	{
		...
		message("channel:rumor", HIM"【武林传奇】某人："+this_player()->query("name")+"获得了六脉神剑谱啦。\n"NOR, users());
	}
	call_out("do_back", 5, me );
