...
string ask_me()
{
	...
	say("杨逍说道：好吧，我召集谢韦二法王和五旗使在东侧殿集合，你稍候到前边找我。\n");

	me->set("assigned_fighter", fighter->query("id"));
	
	if( !fighter->query_temp("fanyao-asked") )
	say("杨逍又道：请你速去告知范右使，请他即刻招集教主殷法王和五散人于西侧殿集合。\n");
	...
	message("vision","杨逍走了过来，跟" + monk->query("name") + "俯耳轻声说了几句。\n", room, monk);
...
int waiting(object me)
{
	...
	if( !objectp( fighter = present( me->query("assigned_fighter"), environment(me)))){
		...
		if( random(10) == 0 ) 
		say("杨逍说道：" + RANK_D->query_respect(fighter) + "去请了范右使没有？ 照理他现在该到了罢？\n");
...
int preparing(object me, object fighter)
{
	...
	message("vision",HIY"\n杨逍大声宣布：东光明圣火阵合围！\n" NOR, room);
	message("vision", "只听四周响起沙沙的脚步声，三法王五散人五旗使们个个神情肃杀，\n"
...
int do_recruit(object me, object fighter)
{
	...
	if (!(ft_fam = fighter->query("family")) || ft_fam["family_name"] != "明教"){
		command("bow");
		say("杨逍说道：" + RANK_D->query_respect(fighter) + "此番过阵，当对本身修行大有助益，\n百尺竿头，更进一步，杨逍就此别过！\n");
		call_out("do_back", 5, me );
	}
	else
	{
		say("杨逍说道：恭喜你闯过了光明圣火阵！本教有此等杰出弟子，届时必当为武林放一异彩！\n");
		call_out("do_back", 5, me );
	}
	return 1;
}
...
int do_back(object me)
{
	...
	message("vision","杨逍挥了挥手， 五旗使和二法王随即鱼贯离开前殿。\n", environment(me) );
