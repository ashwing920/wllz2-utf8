...
string ask_me()
{
	...
	say("范遥说道：好吧，我召集教主殷法王和五散人在西侧殿集合，你稍候到前边找我。\n");

	me->set("assigned_fighter", fighter->query("id")); 		
	
	if( !fighter->query_temp("yangxiao-asked") )
	say("范遥又道：请你速去告知杨左使，请他即刻带同谢韦二法王和五旗使到东侧殿来。\n");
	message_vision("范遥往南离开。\n\n", fighter);
...
int preparing(object me, object fighter)
{
	...
	message("vision", HIY "\n范遥大声宣布：西光明圣火阵合围！\n\n" NOR, room);
	command("chat " + fighter->query("title") + fighter->query("name") + "于今日" + NATURE_D->game_time() + "挑战明教光明圣火阵！\n");
...
int do_recruit(object me, object fighter)
{
	...
	if (!(ft_fam = fighter->query("family")) || ft_fam["family_name"] != "明教"){
		command("smile");
		say("范遥说道：" + RANK_D->query_respect(fighter) + "武功卓绝，兼又手下留情，\n品德更是过人一筹，范遥不胜佩服之至！\n");
		call_out("do_back", 5, me );
	}
	else{
