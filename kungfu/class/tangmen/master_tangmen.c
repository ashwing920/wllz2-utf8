...
#endif
#define FAMILY_MASTER   NPCDATA + "tangmen";
...
void create()
{
	seteuid(getuid());
	if (!restore()) {
		set_name(npc_name("男性"),({ "master_tangmen","master",}));
		set("family/family_name","唐门");
		set("gender", "男性" );
		set("age", 40);
...
int do_copy(object me, object ob)
{
	...
	me->set("short", me->query("title") + " " + me->query("name") + "[Master_tangmen]");
	me->delete("title");
