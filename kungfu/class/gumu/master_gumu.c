...
#endif
#define FAMILY_MASTER   NPCDATA + "gumu";
...
void create()
{
	seteuid(getuid());
	if (!restore()) {
		set_name(npc_name("男性"),({ "master_gumu","master",}));
		set("family/family_name","古墓新派");
		set("gender", "男性" );
		set("age", 40);
...
int do_copy(object me, object ob)
{
	...
	me->set("short", me->query("title") + " " + me->query("name") + "[Master_gumu]");
	me->delete("title");
