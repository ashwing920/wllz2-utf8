...
int practice_skill(object me)
{
        ...
        if (me->query_skill("jiuyin-baiguzhao", 1) < 60)
...
{
        int skill_level;
        skill_level = (int)me->query_skill("huagu-mianzhang",1);

        if(!me->query_temp("weapon") 
        && !me->query_temp("secondary_weapon")
        && me->is_fighting(victim) 
        && victim->is_fighting(me)
        && random(skill_level) > 150 
        && me->query("neili") > 1000 
        && living(victim)){
                me->add("neili",-500);
                message_vision(HIY+"\n只见$N不慌不忙，但掌心中却不停的淡出阵阵清烟，向$n袭去....\n"+NOR,me,victim);
                COMBAT_D->do_attack(me, victim, me->query_temp("weapon"), TYPE_REGULAR);
                victim->apply_condition("hg_poison",victim->query_condition("hg_poison") + 40);
        }
        return 1;
}
