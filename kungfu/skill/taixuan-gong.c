...
{
        if (random(me->query_skill("taixuan-gong",1)) > 100 && me->query("neili") > 300)
        {
                me->add("neili", - 200);
                return ([
                        "action": "$N的心中突然一片茫然，只觉得经脉中象有无数的蝌蚪在串来串去，难过非常，\n当下不及多想，顺着蝌蚪的走向一阵拳打脚踢！四周顿时劲风乱飑，好不热闹！",
                        "damage": 1000,
                        "dodge" : 100,
                        "force" : 400,
                        "damage_type": "瘀伤",
...
{
        if(random(this_player()->query("int")) > 25) return 1;
