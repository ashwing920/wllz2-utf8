...
{
	int lvl;
	lvl = (int)me->query_skill("sanyin-unarmed",1);
	if( damage_bonus < 100 ) return 0;
	if( me->query_temp("weapon") || me->query_temp("secondary_weapon")) return 0;
	if( lvl > 100 && me->query("neili") > 200 )
	if( (int)me->query("neili") > random(victim->query("max_neili"))) {
		victim->receive_wound("qi", (damage_bonus - 100) / 4 , me);
		message_vision(HIR"只听见“唰”的一声轻响，$n的身上被划开了一道血口子！\n"NOR,me,victim);
		victim->apply_condition("sanyin_poison",lvl / 2 );
		me->add("neili",-100);
		return 1;
	}
}
