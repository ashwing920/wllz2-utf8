// xuan.c
// 冰雪纷飞

#include <ansi.h>

inherit F_SSERVER;
 
int perform(object me, object target)
{
        object weapon;
        string msg,weapon_name;
        int i,lvl,attack_time;
        
        lvl = (int)me->query_skill("tianshan-zhang",1);
        if( lvl <= 150) attack_time = 4;
        if( lvl <= 250 && lvl > 150 ) attack_time = 6;
        if( lvl <= 350 && lvl > 250 ) attack_time = 8;
        if( lvl > 350 ) attack_time = 12;

        if( !target ) target = offensive_target(me);
        if( !target || !target->is_character() || !me->is_fighting(target))
                return notify_fail("「冰雪纷飞」只能在战斗中使用。\n");

        if (!objectp(weapon = me->query_temp("weapon"))
                || (string)weapon->query("skill_type") != "staff")
                        return notify_fail("你使用的武器不对。\n");

        if((int)me->query("neili") < 1000)
                return notify_fail("你的内力修为不够, 不能使用这一绝技！\n");
        if((int)me->query_skill("tianshan-zhang",1) < 150)
                return notify_fail("你的天山杖法修为不够，目前不能使用冰雪纷飞！\n");
        if((int)me->query_skill("huagong-dafa",1) < 150)
                return notify_fail("你的化功大法修为不够，目前不能使用冰雪纷飞！\n");
        if( (int)me->query("neili") < 400)
                return notify_fail("你的内力不够！\n");

        weapon_name = weapon->query("name");
        msg = HIW"$N使出天山杖法中的绝技「"HIB"冰雪纷飞"HIW"」，手中"+ weapon_name +HIW"顿时飞起数道白幌，向$n袭去！\n" NOR;
        message_vision(msg, me,target);
        if( random(me->query("combat_exp")) > (int)target->query("combat_exp") / 4) {
                msg = HIC "结果$p被$P攻了个措手不及！\n" NOR;
                message_vision(msg, me,target);
        for(i = 0; i < attack_time; i++)
                if (me->is_fighting(target) && target->is_fighting(me) && target->query("eff_qi")>0){
                me->set_temp("action_flag",1);
                if (!weapon->query("equipped")) break;
                COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 0);
                me->set_temp("action_flag",0);
        }else break;
                me->add("neili", -400);
                me->start_busy(4);
        }
        else {
                msg = HIY"$p轻身闪开，躲避了$P凶猛的招式。\n" NOR;
                message_vision(msg, me,target);
                me->start_busy(5);
        }
        return 1;
}
