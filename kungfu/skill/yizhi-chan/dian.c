        ...
        if( random( me->query_skill("force") ) > random((int) target->query_skill("force") ) ){
                ...
                if (! objectp(ob)){
                        ...
                        if (target->query_skill("dodge") > 200 && random(2) == 0){
                                switch (random(2)){
                                case 0:
                                        msg += HIR "$p连续变了几种上乘身法，终于没能躲过，$N的这一指正点在$p的丹田上，\n"NOR;
                                        msg += HIR "$p只觉得一口凉气，自丹田而上，心里有种说不出的难受。\n"NOR;
                                        damage = 3;
                                        break;
                                case 1:
                                        msg += HIR "$p轻身飘开，身法灵动之极，众人正要喝彩，听「噗」的一声，如击败革，$p脸色煞白，摇摇欲坠。\n"NOR;
                                        damage = 5;
                                        break;
                                }
                        } else
                        if (target->query_skill("force") > 200 && random(2) == 0)
                        {
                                ...
                                {
                                case 0:
                                        msg += HIR"$p冷哼一声，自持内力高强，反手出招，「啵」的一声二指相交，$p身行微晃，脸如金纸。\n"NOR;
                                        damage = 3;
                                        break;
                                case 1:
                                        msg += HIR"$p一声长啸，双手幻化出无数掌影，护住身前。只听「嗤」的一声，掌影顿消，$p已经变得脸色惨白。\n"NOR;
                                        damage = 4;
                                        break;
                                }
                        } else
                        {
                                ...
                                {
                                case 0:
                                        msg += HIR"$p看到这招，竟然不知躲避，「嗤」的一声，$N这一指正点在$p的檀中大穴。\n"NOR;
                                        target->unconcious();
                                        damage = 0;
                                        break;
                                case 1:
                                        msg += HIR"$p一转念，已经想出了九十九种化解的办法，尚未来得及施展，\n"NOR;
                                        msg += HIR"只听「嗤」的一声，$p已被$N点中檀中大穴。\n"NOR;
                                        target->unconcious();
                                        damage = 0;
                                        break;
                                }
                        ...
                        if (damage > 0)
                                target->receive_damage("qi",damage * 12,me);
