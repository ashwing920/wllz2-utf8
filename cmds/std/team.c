...
int main(object me, string arg)
{
	...
	if(sscanf(arg,"dismiss %s",arg)==1) {
		...
		|| !living(ob) || !ob->is_character() || ob==me || !me->visible(ob))
			return notify_fail("你想将谁逐出队伍？\n");
		if( !pointerp(me->query_team()))
			return notify_fail("你并没有组成队伍。\n");
		if( !me->is_team_leader())
			return notify_fail("你并不是队伍领袖，没资格将队伍中的人进行驱逐。\n");
		if(member_array(ob,me->query_team()) == -1)
			return notify_fail(ob->name(1)+"并没有加入你的队伍。\n");
		if(me->dismiss_team(ob)){
			ob->dismiss_team();
			message_vision("$N将$n逐出队伍。\n", me, ob);
			if( sizeof(me->query_team()) == 1 && member_array(me,me->query_team()) != -1){
				me->dismiss_team();
				message_vision("$N将队伍解散了。\n",me);
			}
			return 1;
		}
	}
...
team dismiss <某人>  - 解除队伍当中的某一个人，只有队伍的领袖才可以使用这个指令。
team form <阵法>     - 如果队伍的领队学过阵法的话，可以将队伍中的成员组织成某种阵
		       形，组成阵形的状态只要领袖一移动就会解除。
