...
int main(object me, string arg)
{
	...
	if( stringp(msgg = obj->query("env/no_tell")))
		no_tell_msg = CYN+ obj->name(1) + msgg + NOR"\n";
	else
		no_tell_msg = "对方不想听你罗嗦，有事请用聊天频道。\n";

	if(obj->query("env/no_tell") && !wizardp(me)){
		if(pointerp(frd_list)){
			if(member_array(me->query("id"),frd_list) == -1)
				return notify_fail(no_tell_msg);
		}
		else {
			return notify_fail(no_tell_msg);
		}
