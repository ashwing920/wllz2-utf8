...
int main(object me, string arg)
{
	...
	if((int)environment(me)->query("no_fight") )
	...
	if( userp(obj) && !environment(obj)->query("fighters_room"))
	...
	if(t[i]){
		...
		obj->kill_ob(t[i]);
		if( userp(obj) ) {
//加入判断凶手by zen 99-02-25
		if(!obj->query_temp("first_kill/"+t[i]->query("id")) )
		t[i]->set_temp("first_kill/"+obj->query("id"), 1);
		}
		}
	}
	return 1;
