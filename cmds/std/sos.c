...
void done_post(object me, mapping note, string text)
{
	...
	notes += ({ note });
	tell_object(me,HIW"求救完毕。你的信息已经被加到巫师通知队列里。\n");
	tell_object(me,HIW"巫师正在处理其他玩家，请等待。\n");
	tell_object(me,HIW"你可以先忙你自己的事情，过一段时间后可以用sos read查看是否被解决。\n" NOR);
	CHANNEL_D->do_channel(find_object(SEC_OB),"sys",sprintf("%s(%s)发布了一条求救信息，在线巫师请用 sos 指令查阅。",
		me->name(1),geteuid(me)));
	save();
	return;
}
void done_re(object me, mapping note, int i,string text)
{
	string s;
...
void list_bug(object me)
{
	...
	i+1, notes[i]["title"], notes[i]["solve"]==""?"待处理":"处理完成："+notes[i]["solve"],
...
void do_read(object me, int i)
{
	...
	s = sprintf("[%d] %-50s %10s %s\n"
		  "------------------------------------------------------------------------\n"
		  "%s",
		   i, notes[i-1]["title"], notes[i-1]["solve"]==""?"待处理":"处理完成："+notes[i-1]["solve"],
...
//	if (file_size(SOS_DIR + SOS_FILENAME) == -1) {
  //	  notes = ({ });
	...
	if (!arg) {
	if(!wiz_level(me)){
		tell_object(me,HIW"你正在" HIY+ code_name() + HIW"里辛苦地挖着泥巴，突然发现一只大个的臭虫向你爬来。\n" NOR);
		tell_object(me,HIW"你赶紧向巫师们发出紧急求救信号：巫师救命呀！！！\n" NOR);
	...
	if( sscanf(arg, "solve %s",arg1) == 1 ) {
		if( !wiz_level(me) ) 
			return notify_fail("格式不对，具体使用方法请参阅 help sos \n");
		if( arg1 =="all") {
			if( !(i = sizeof(notes)) ) 
				return notify_fail("没人发出sos求救信号。\n");
			for( ; i > 0; i-- )
				notes[i-1]["solve"] = me->query("id");
