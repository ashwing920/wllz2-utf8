...
int main(object me, string arg)
{
	...
	if (wizardp(target)) 
		return notify_fail("连巫师都敢暗算，你活腻了？\n");
	...
	if((int)target->query("age") <= 18 && userp(target))
		return notify_fail("为了世界更美好，放过小孩子吧.\n");

	if(userp(target) && (int)me->query("age") <= 18)
		return notify_fail("算了吧，你这不是去送死嘛？\n");
	...
	me->add("jing", -50);
	if(userp(target)){
		write_file("/log/static/ATTEMP_ANXUAN",sprintf("[%s]%s[%s]试图暗算%s[%s]\n",
			ctime(time()),me->name(1),me->query("id"),target->name(1),target->query("id")));
	}
	return 1;
}
