...
int vote(object me, object victim)
{
	...
	{
		return notify_fail(victim->query("name")+"的频道已经是关闭的了。\n");
	}
	...
	if (df>1)
	{
		shout( HIG "【表决】"+me->query("name")+"投票关闭" +victim->query("name")+"的交谈频道，还差"+sprintf("%d", df)+"票。\n" NOR);
		write( HIG "【表决】"+me->query("name")+"投票关闭" +victim->query("name")+"的交谈频道，还差"+sprintf("%d", df)+"票。\n" NOR);
		victim->apply_condition("vote_clear", 10);
	} else
	{
		shout( HIG "【表决】"+me->query("name")+"投票关闭" +victim->query("name")+"的交谈频道。"+victim->query("name")+"的交谈频道被关闭了！\n" NOR);
		write( HIG "【表决】"+me->query("name")+"投票关闭" +victim->query("name")+"的交谈频道。"+victim->query("name")+"的交谈频道被关闭了！\n" NOR);
		victim->apply_condition("vote_clear", -10);
		victim->set("chblk/all",1);
		victim->apply_condition("be_chblk",600);
	}
	return 1;
}
