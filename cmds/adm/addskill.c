...
int main(object me, string arg)
{
	...
	if( skill == "all" ) {
		...
		for(i=0; i<j; i++) {
			if( level ==0 ) {
				...
				ob->set_skill(skills[i],skill_status[skills[i]] + level);
				write(sprintf("addskill-> %-36s%d\n", to_chinese(skills[i]) +
					"(" + skills[i] + ")", level));
			}
		}
	}
	...
	if( userp(ob) && ob!=me ) log_file("addskill",
...
用来设定某人的某种技能的级别数, 如果第二项是 all 则将其所有的技
能改为设定的级别数。如果选择了级别数为 0 则删除该人此项技能。
