...
void check(object obj, object dest)
{
	...
	shout( HIR"【比武精灵】比武正式开始，由" + name1 + "对" + name2 + "！\n"NOR);
	write( HIR"【比武精灵】比武正式开始，由" + name1 + "对" + name2 + "！\n"NOR);
...
int observe(object obj,object dest)
{
	...
	{
		shout(HIR"【比武精灵】" +obj->query("name") + "落荒而逃了！\n"NOR);
		write(HIR"【比武精灵】" +obj->query("name") + "落荒而逃了！\n"NOR);
		return 1;	}

	if ( !present(dest, environment(this_player())) ) 
	{
		shout( HIR"【比武精灵】" +dest->query("name") + "落荒而逃了！\n"NOR);
		write( HIR"【比武精灵】" +dest->query("name") + "落荒而逃了！\n"NOR);
		return 1;
	}
	...
	{
		obj->delete_temp("halted");
		shout( HIR"【比武精灵】"+ obj->query("name") +"中途放弃比武！\n"NOR);
		write( HIR"【比武精灵】"+ obj->query("name") +"中途放弃比武！\n"NOR);
		return 1;
	}
	if (dest->query_temp("halted"))
	{
		dest->delete_temp("halted");
		shout( HIR"【比武精灵】"+ dest->query("name") +"中途放弃比武！\n");
		write( HIR"【比武精灵】"+ dest->query("name") +"中途放弃比武！\n");
		return 1;
	}
	...
	if ( obj->query("qi")*2 > obj->query("max_qi")) 
	{
		if  (dest->query("qi")*2 > dest->query("max_qi")) {
			shout( HIR"【比武精灵】" + obj->query("name") 
				+ "与" + dest->query("name") + "比武不分胜负！\n"NOR);
			write( HIR"【比武精灵】" + obj->query("name") 
				+ "与" + dest->query("name") + "比武不分胜负！\n"NOR);
			return 1;
		}
		shout( HIR"【比武精灵】"+ obj->query("name") 
	...
	{
		shout( HIR"【比武精灵】" + dest->query("name") 
			+ "比武战胜" + obj->query("name") + "！\n"NOR);
		write( HIR"【比武精灵】" + dest->query("name") 
			+ "比武战胜" + obj->query("name") + "！\n"NOR);
	}
