...
int main(object me, string arg)
{
	...
	lvl = wiz_level(me);
	if( lvl < wiz_level(obj) ) return notify_fail("找不到指定的物件。\n");

	if( objectp(obj) ){
		log_file("static/CALL_PLAYER",
			sprintf("%s call %s->%s(%s) on %s\n",
				me->name(2),obj->name(2),func, param,ctime(time())));

		CHANNEL_D->do_channel(find_object(SEC_OB),"sys",
			me->name(2)+HIW"CALL"HIR+obj->name(2)+"->"+func+"("+param+")");
