...
int main(object me, string file)
{
	...
	if (err)
		...
		write("成功！\n");
		CHANNEL_D->do_channel(find_object(SEC_OB),"sys",sprintf("%s[%s] UPDATE [%s]",
		me->query("name"),geteuid(me),resolve_path(me->query("cwd"), file)));
		log_file("file/UPDATE", sprintf("%s[%s] UPDATE [%s] (%s)\n",
			me->query("name"),geteuid(me),resolve_path(me->query("cwd"), file),ctime(time())));
