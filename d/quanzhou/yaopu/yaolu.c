...
#include "/d/quanzhou/yaopu/drug_list.h"
...
int do_open(string arg)
{
	...
	if (query("is_burning"))
	{
		...
		me->receive_damage("qi",150+random(50));
		me->receive_damage("jing",150+random(50));
		me->set_temp("last_damage_from","被药炉烫死了。");
		set("long",_DES+_FIRE);
		return 1;
	}
...
int do_addyao(string arg)
{
	...
	if (!objectp(obj = present(item, me)))
		return notify_fail("你要放什么东西到药炉里？\n");
...
int do_aoyao()
{
	...
	message_vision(HIC"$N一手靠近炉火一抽一放，借内力操控火势；一手握住药杵，从盖上孔中伸入不停搅拌。\n"NOR,me);
	message_vision(HIR"\n$N不停的重复运动，一会儿已是满身大汗。\n"NOR,me);
...
void finish(object me)
{
	...
	message_vision(HIY"$N深吸一口气，停了下来。\n"NOR,me);
...
int do_quyao(string arg)
{
	...
	{
		message_vision("$N开炉一看，炉中之物象一团浆糊一样，看来没什么用。\n",me);
	}
	else
	{
		ob = new("/clone/yaowu/"+query("drug"));
