...
void create()
{
	set("short", "山洞");
	set("long", @LONG
对面石壁上斜倚着一副骷髅骨，身上衣服已烂了七八成，那骷髅骨宛
然尚可见到是个人形。见石室中别无其他可怖事物。骷髅前面横七竖八的
放着十几把金蛇锥，石壁上有几百幅用利器刻成的简陋人形(map) 。图形
尽处，石壁上出现了十六个字(word)，也是以利器所刻。十六字之旁，有
个剑柄凸出在石壁之上，似是一把剑插入了石壁，直至剑柄。
LONG
);
	set("outdoors", "jinshe");
	set("sword_count", 1);
...
int do_pull(string arg)
{
	...
	{
		message_vision("$N好奇心起，握住浚柄向外一拔，却是纹丝不动，竟似铸在石里一般。\n", me);
		me->add("neili",-10);
		return 0;
	}
	ob = new(__DIR__"obj/jinshe-jian");
	ob->move(me);
		message_vision("$N紧紧握住浚柄，潜运内力，嗤的一声响，拔了出来，剑柄下果然连有剑身。\n", me);
	me->add("neili",-1000);
	add("sword_count", -1);
	return 1;
}
