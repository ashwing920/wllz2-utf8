...
void create()
{
	set("short", "佛堂");
	set("long", @LONG
这是一间装饰得十分讲究的佛堂，墙和地板都是砖 (brick)砌的。佛
堂里供奉着一尊如来佛像 (foxiang)，佛像前是一张供桌。桌上点着盏油
灯。屋里有些昏暗。
LONG
);
	set("outdoors", "city2");
...
int do_unlock(string arg)
{
	...
	{  
		message_vision("$N猛地一拉铁环，蓦听的飕的一声，一枝机弩从下面射了出来，正中$N胸口。\n", me);
		me->die();
		return 1;
	}
	...
	"正是一部《四十二章经》。$N忍不住啃了一声。\n",me);
		...
		destruct(present("jin yaoshi",me));
		set("getbook",1);
		ob->move(this_player());
	CHANNEL_D->do_channel(find_object(MASTER_OB),"rumor",sprintf("听说了吗？%s拿到%s了！",
		me->name(),ob->name()));
	return 1;
}
