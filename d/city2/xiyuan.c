...
void create()
{
        set("short", "长安戏院");
        set("long", @LONG
这里就是著名的长安戏院，几个戏子在台上(stage) 唱戏。台下黑压
压的一片都是人。唱到妙处，观众们禁不住大声喝采。东边是后台。
LONG
);
        set("exits", ([
...
int valid_leave(object me, string dir)
{
        ...
        if(dir=="up" && userp(me)){
                ...
                message_vision(  "只听哗啦啦一片掌声，观众们一起鼓掌。有人大声说道：静一静，请看这位" + RANK_D->query_respect(me)+ "表演。\n",this_player());
        ...
        for(i=0; i<sizeof(ob); i++) {
                if( !userp(ob[i]) && ob[i]->query("id") != "guan zhong" ) { 
                        ob[i]->set_leader(me);
                        ob[i]->kill_ob(me);
                        me->fight_ob(ob[i]);
                        }
