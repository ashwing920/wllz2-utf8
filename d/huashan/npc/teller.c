...
int do_forge()
{
        ...
        write("李铁嘴轻声跟你说道：不是我吹牛，保证蓦的跟原来字体一模一样，\n每封信五十两白银，不二价。\n");
...
int accept_object(object who, object ob)
{
        ...
        if( ob->value() >= 5000 ) {
                 if( who->query_temp("marks/李4") ) {
                        write("李铁嘴对你说道：这封信是造好了，但" + RANK_D->query_respect(who) + "要是出了事，千万别把我说出来呀。\n");
                ...
                say("李铁嘴一面急忙把钱收起来，一面笑嘻嘻说道：" + RANK_D->query_respect(ob) +"，这怎么敢当。\n");
                return 1;
        }
        if( ob->value() >= 500 ) {
