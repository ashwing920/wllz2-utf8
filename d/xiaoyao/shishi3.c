...
void create()
{
	set("short", "石室");
	set("long", @LONG
这里是一个极大的石室，比之外面的任何一个都大了数倍，室中一排
排地列满了木制成的书架(shelf),可是书架上却空荡荡地连一本书也没有
，走近看时，只见书架上贴满了“昆仑派”，“少林派”，“青城派”之
类的条目，想象当年所列，皆是各门各派的武功秘籍。室中还有一张小床
(bed)。
LONG
);
	set("exits", ([
...
int do_strike(string arg)
{
	...
	if( random(50) > (int)me->query("kar"))
	{
		message_vision("忽然，$N听到一个声音嘿嘿地奸笑起来：“你上当啦！”\n",me);
		message_vision("接着，不知道哪里射出一支毒箭，射中了$N！\n",me);
		me->die();
	}
	if(random(50) == (int)me->query("kar") && !present("miji1",me) && query("book1_count")>=1)
	{
		add("book1_count", -1);
		me=new("/clone/book/strike_book");
		me->move(__DIR__"shishi3");
		CHANNEL_D->do_channel(find_object(MASTER_OB),"rumor",sprintf("%s得到掌法总谱啦！",me->name()));
