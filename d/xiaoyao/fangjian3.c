...
void create()
{
	set("short", "房间");
	set("long", @LONG
你只觉得眼前陡然一亮，一个宫装的美女，正拿着一把长剑，对准了
你的胸膛！……一场虚惊之后，你才发现原来只是个白玉雕成的玉像，但
是看上去却与真人一般无异。玉像的一双眸子莹然有光，神采飞扬。你此
时只觉得神驰目眩，竟如着魔中邪，眼光再也离不开玉像……
LONG
);
...
int do_kneel(string arg)
{
	...
	me = this_player();
	if( !arg || arg == "" || arg != "putuan") 
		return notify_fail("你要跪在哪里？\n");
	if(!present("putuan",environment(me)))
		return notify_fail("这里没有蒲团。\n");
...
int do_ketou()
{
	...
	if( random(100) == 37 && !present("bojuan", me) && query("book_count") >= 1)
	{
		add("book_count", -1);
		...
		CHANNEL_D->do_channel(find_object(MASTER_OB),"rumor",sprintf("%s拿到北冥神功帛卷啦！",me->name()));
	}
	return 1;
