...
。龙首刻一香炉，在此上香称为“上龙头香”，是香客的最大心愿。石柱
仅粗如屋梁，旁无扶栏，前有白云缭绕，下有万丈深渊，稍一失足，即尸
骨无存。
LONG
);
	set("exits",([ /* sizeof() == 1 */
...
int do_mianbi(string arg)
{
	...
	if (((int)ob->query_skill("huagong-dafa", 1))
	     ||	((int)ob->query_skill("huntian-qigong", 1))
	     ||	((int)ob->query_skill("hunyuan-yiqi", 1))
	     ||	((int)ob->query_skill("linji-zhuang", 1))
	     ||	((int)ob->query_skill("xuantian-force", 1))
	     ||	((int)ob->query_skill("zixia-shengong", 1))){
	  message_vision("$N旁门杂学太多了，实在静不下心来面壁。\n",ob);
	  return 1;
	}    
	if (arg == "liangyi-jian" || arg == "tiyunzong"){
