...
void create()
{
	set("short", "石室");
	set("long", @LONG
这里火光昏暗，只见那龙岛主与木岛主盘膝坐在锦垫之上，面对石壁
(wall)，凝神苦思。此石室已是山腹最深处，却宽阔高大，足可容下近百
人。洞中火把亦是其他洞的二三倍之多。
LONG );
	set("exits", ([
	...
	set("item_desc", ([
		"wall" : @WALL
墙的正上方刻着「白首太玄经」几个大字。字的下方刻的是密密麻麻的蝌
蚪文，成千上万。文字排列甚不整齐，或横或竖，或直或斜。左下的大片
甚至混成一团难分次序，委实难以辨认。
WALL
	]));
	set("objects", ([
...
int do_study(string arg)
{
	...
	for (i = 1; i<=times; i++)
	{
		...
		if(((int)me->query("str") + (int)me->query("con") + (int)me->query("dex")
		+ (int)me->query("int") + (int)me->query("per") + (int)me->query("kar")
		+ (int)me->query("spi") + (int)me->query("cor")) < 240 
		|| (int)me->query("kar") < 25){
