...
void create()
{
	...
	set("material", "steel");
	set("long",@LONG
这是桃花岛布阵专用的道具：「布阵箱」。里面有各种各样的道具，可以用来布阵。
LONG
);
	}
	setup();
...
int do_array(string arg)
{
	...
	if(obj == me)
		return notify_fail("你想自杀？\n");

	myexp=(int)me->query("combat_exp");
	objexp=(int)obj->query("combat_exp");
	mylev=(int)me->query_skill("qimen-wuxing");
	objlev=(int)obj->query_skill("qimen-wuxing");
	...
	me->start_busy(4);
	if(userp(obj)) me->set_temp("first_kill/"+obj->query("id"), 1);
	if( userp(obj) ) {
		obj->fight_ob(me) ;
		me->kill_ob(obj);
	}               
