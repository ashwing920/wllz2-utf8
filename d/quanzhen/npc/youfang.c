...
int accept_object(object me, object ob)
{
	...
	if( me->query_temp("ask_qifu") && ob->query("money_id") && ob->value() >= 100){
		command("say "+ RANK_D->query_respect(me) +"如此心善，贫僧自当为"+
		RANK_D->query_respect(me)+"日夜颂经，消灾解难！！");
		remove_call_out("shaqi");
		call_out("shaqi",random(40),me);
		return 1;
	}
