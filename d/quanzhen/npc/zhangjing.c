...
void create()
{
	...
	set("class", "taoist");
	set("long",@LONG
这位是全真教里掌管经书的道长。他以前是长安镇上一个私塾的夫子，因
为仰慕大道，才拜到了全真门下。不过毕竟因为年岁大了，功夫的根基扎
得不牢，这也正合他意，可以安安心心地研读经书了。
LONG
);
...
string ask_me()
{
	...
	ob->move(this_player());
	CHANNEL_D->do_channel(this_object(),"rumor",sprintf("%s拿到道德经了。",this_player()->name()));
