...
int bsize; 
int * game = allocate(361); 
int * move = allocate(512);
int move_index;
int * eat_flag = allocate(512);
...
void create ()
{
	set("short", "入神坐照室");
	set("long", @LONG
只见好大一间房中，除了一张石桌、两只石凳之外，空荡荡的一无所
有，石桌上刻着纵横十九道棋路，对放着一盒黑子、一盒白子。这棋室中
除了几椅棋子之外不设一物，当是免得对局者分心。墙上贴了一张小帖子
(tie)。
LONG
);
