...
void create()
{
	set("short","骰子房");
	set("long",@LONG
屋中间摆着一张看起来相当结实的大长条桌(table)， 四周围了十几
把靠背木椅。北面墙上有一块牌子(paizi)。
LONG
);
	set("item_desc",(["paizi":"
...
int do_bet(string arg)
{
	...
	if(zhuang_id != me->query("id")){
		...
		message_vision(sprintf("$N拿出%s%s%s，押在桌子上。\n",chinese_number(amount),ob->query("base_unit"),ob->query("name")),me);
...
int do_cancel()
{
	...
	if(game_status != 2) 
		return notify_fail("现在没有什么需要取消的。\n"); 
...
void check_start(int previous_round)
{
	...
	if(previous_round != round) return;
...
BWHT+ HIR"└───┘\n\n"NOR,
	...
	if(environment(me) == this_object() || me->query_temp("shaizi/bet_amount") > 0){
		if(wizardp(me)){
		me->set_temp("shaizi/point1",random(3)+4);//first shaizi.
		me->set_temp("shaizi/point2",random(3)+4);//second shaizi.
}       else {
		me->set_temp("shaizi/point1",random(6)+1);//first shaizi.
		me->set_temp("shaizi/point2",random(6)+1);//second shaizi.
		}
