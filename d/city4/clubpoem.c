...
古论今，你瞧，不知谁在茶桌上刚刚写好一首诗(poem)。里边柜台上新贴
了张启事(qishi)，你不由得走过去看了几眼。
LONG);
...
int do_answer(string arg)
{
        ...
        if(me->is_fighting())
                return notify_fail("一边打架一边做诗？真有兴致。\n");

        if(me->query("jing") < 20)
                return notify_fail("你现在不清醒，还是等一下再说吧。\n");

        if( me->query_skill("literate",1) < 10)
                return notify_fail("你字都不识半个，就跑这里来瞎扰和了？\n");

        if( me->query("combat_exp") > 1000000)
                return notify_fail("依你现在能力，还是做点别的吧！\n");
        ...
        switch(random(3)) {
                ...
                break;
                case 2: message_vision("$N疑道：是不是“"+ arg +"”呀？\n",me);
                break;
        }
...
void poem_reward1(object me)
{
        int dx,pot,lite,silver;
        object money;

        switch(random(4)) {
        case 0:
                dx= me->query("kar") + random(100);
                me->add("combat_exp",dx);
                write("你答对了茶博士的题目，获得了一些经验做为奖励！\n");
                break;
        case 1:
                pot= me->query_int() + random(100);
                me->add("potential",pot);
                write("你答对了茶博士的题目，获得了一些潜能做为奖励！\n");
                break;
        case 2:
                lite=4+random(40);
...
void do_test()
{
        ...
        if(objectp(ob=present("cha boshi",this_object())) && living(ob)) {
...
void copy_to_old()
{
        ...
        || objectp(ob=present("li bai",this_object())))
...
{
        string result;
        string *color=({BLK,RED,GRN,YEL,BLU,MAG,CYN,WHT,HIR,HIG,HIY,HIB,HBRED,HBGRN,HBYEL,HBBLU,HBMAG,BBLK,BRED,BGRN});
        int len,i;
        len=sizeof(color);
        result="";
        i=sizeof(arg);
