...
{  
        ...
        if (i_dodge < 100 ) return notify_fail("莲花峰对于你来说太高了\n");
                ...
                message("vision","只见人影一晃"+me->name() + "奋不顾身向下跳去。\n",environment(me), ({me}));
                me->move(__DIR__"lianhua");
                message("vision","只见人影一闪"+me->name() + "从下面跳了上来。\n",environment(me), ({me}));
...
int do_zuan( string arg)
{
        ...
        if( me->query("gender") == "无性" )
                return notify_fail("你刚想往洞里面钻，忽然想起来，这种地方一般都是男女幽会的地方，象我这种人，还是别去的好。\n");
        if ( me->query("gender") == "女性" && (int)me->query("age")<25 ){
                message("vision",me->name() + "一弯腰，钻进了小洞。\n",environment(me), ({me}) );
                me->move(__DIR__"xiaodong");
                message("vision",me->name() + "从从外面钻了进来。\n",environment(me), ({me}) );
                message("vinson","你一弯腰，向洞里面钻去，洞口很小，幸好你是女孩子，身子小，不费什么力就钻进去了。\n",me);
        }
