...
string ask_me()
{
	...
	obj = new("/clone/book/xuanming_book");
	obj->move(me);
	message_vision("$N对$n点了点头，说道：这本《玄冥掌谱》你就拿去好好参详吧，秘籍只有一本，记得切勿丢失了。\n",this_object(),me);
	me->set("zongshi_gift",1);
	me->save();
	log_file("zongshi_gift",sprintf("[%s] %s[%s]从百人道场获得了一本%s。\n",ctime(time()),me->name(1),me->query("id"),obj->name(1)));
	return CHANNEL_D->do_channel(this_object(),"rumor",sprintf("%s，%s从百人道场获得了一本《玄冥掌谱》。",NATURE_D->game_time(),me->name(1)));
