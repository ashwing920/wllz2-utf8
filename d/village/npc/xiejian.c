...
int do_name(string target)
{
	...
	me->set("long","邪剑李四，杀手帮十大高级剑士之一，有剑不染血不归鞘之名。\n");
	me->delete("chat_chance");
	me->delete("chat_msg");
	message("vision",HIY "李四嘿嘿嘿地干笑了几声，说道：您就在这儿静侯好音吧!\n"NOR,environment(), me );
	me->set("pursuer", 1);
...
int do_kill(object me, object dest)
{
	...
	if( objectp(dest) && present(dest, environment(me))
		...
		message_vision(HIR "$N对$n说道：阴司鬼王差我来，请你去他那儿做客，"+RANK_D->query_rude(dest)+"，你认命吧！\n" NOR, me, dest);
		me->set_leader(dest);
		me->kill_ob(dest);
		dest->fight_ob(me);
