...
void create()
{
	...
	if(me && mapp(weapon= me->query("weapon"))){
		if(!weapon["id"]) weapon["id"] = "my hammer";
		set_name(weapon["name"],({ weapon["id"],"hammer"}));
		...
		if(weapon["wield_msg"])
			...
			set("wield_msg", "$N往后腰中一摸，取出了一柄"+ weapon["name"]+"握在手中。\n");
		
		if(weapon["unwield_msg"])
			set("unwield_msg", weapon["unwield_msg"]+"\n");
		else
			set("unwield_msg", "$N手中"+weapon["name"]+"往腰后一别，眨眼间已然不见影踪。\n");

		set_weight(weight() + (weapon["lv"] * 150));
		set("weapon_prop",me->query("weapon/weapon_prop"));
		init_hammer(weapon["lv"] * 10);
	}
	else {
		set_name("传家铁锤",({"my hammer","hammer"}));
		set_weight(500);
		set("unit","把");
		init_hammer(1);
	}
...
int do_duanlian(string arg)
{
	...
	tell_object(me,RED"你手指锤体，一股内力丝丝的传了进去。\n"NOR);
	...
	if( w_zhi >= ((w_level + 1) * (w_level + 1))){
		...
		tell_object(me,HIY"\n"+ w_name +HIY"身忽的一亮，一道金光隐入"+ w_name +HIY"，不见了！\n\n"NOR);
...
int do_hui(string arg)
{
	...
	message_vision(HIR "$N大喝一声，一掌擎锤，一掌猛力击下。结果轰隆一声巨响$n"HIR"裂为两半！\n" NOR,me,this_object());
	me->delete("weapon");
