...
void create()
{
	...
	if(me && mapp(weapon= me->query("weapon"))){
		if(!weapon["id"]) weapon["id"] = "my club";
		set_name(weapon["name"],({ weapon["id"],"club"}));
		...
		if( weapon["wield_msg"])
			...
			set("wield_msg", "$N往腰中一摸，拿出了一把"+ weapon["name"]+"，端在手中。\n");
		
		if( weapon["unwield_msg"])
			set("unwield_msg", weapon["unwield_msg"]+"\n");
		else
			set("unwield_msg", "$N手中"+weapon["name"]+"抖出一个棍花，眨眼间已然不见影踪。\n");

		set_weight(weight() + (weapon["lv"] * 150));
		set("weapon_prop",me->query("weapon/weapon_prop"));
		init_club(weapon["lv"] * 10);
	}
	else {
		set_name("传家木棍",({"my club","club"}));
		set_weight(500);
		set("unit","把");
		init_club(1);
	}
...
int do_duanlian(string arg)
{
	...
	tell_object(me,RED"$N手指棍脊，一股内力丝丝的传了进去。\n" NOR);
	...
	if( w_zhi >= ((w_level + 1) * (w_level + 1)))
	{
		...
		tell_object(me,HIY"\n"+ w_name +HIY"身忽的一亮，似乎一种新生的力量在"+ w_name +HIY"中涌动起来！\n\n" NOR);
...
int do_hui(string arg)
{
	...
	message_vision(HIR "$N大喝一声，一掌擎棍，一掌猛力击下。结果轰隆一声巨响$n"HIR"断为两截！\n" NOR,me,this_object());
	me->delete("weapon");
