...
void create()
{
	...
	if(me && mapp(weapon= me->query("weapon"))){
		if(!weapon["id"]) weapon["id"] = "my staff";
		set_name(weapon["name"],({ weapon["id"],"staff"}));
		...
		if(weapon["wield_msg"])
			...
			set("wield_msg","$N往背后一摸，掏出了一柄"+ weapon["name"] +"握在手中。\n");

		if( weapon["unwield_msg"])
			set("unwield_msg", weapon["unwield_msg"]+"\n");
		else
			set("unwield_msg", "$N手中"+ weapon["name"] +"一挥，眨眼间已然不见影踪。\n");

		set_weight(weight() + (weapon["lv"] * 150));
		set("weapon_prop",me->query("weapon/weapon_prop"));
		init_staff(weapon["lv"] * 10);
	}
	else {
		set_name("传家之杖",({"my staff","staff"}));
		set_weight(500);
		set("unit","柄");
		init_staff(1);
	}
...
int do_duanlian(string arg)
{
	...
	tell_object(me,RED"你手握杖端，一股内力丝丝的传了进去。\n"NOR);
	...
	if( w_zhi >= ((w_level + 1) * (w_level + 1))){
		...
		tell_object(me,HIY"\n"+ w_name + HIY"身忽的一亮，一道金光隐入你的"+ w_name +HIY"，不见了！\n\n"NOR);
...
int do_hui(string arg)
{
	...
	message_vision(HIR"$N大喝一声，一掌擎杖，一掌猛力击下。结果轰隆一声巨响$n"HIR"断为两截！\n" NOR,me,this_object());
	me->delete("weapon");
	me->save();
	destruct(this_object());
	return 1;
