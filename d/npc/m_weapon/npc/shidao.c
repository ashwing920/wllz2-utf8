...
{       
	...
	message("vision","侍刀低头检视了一下手中的"+(string)ob->query("name")+"点了点头说：那好！我们现在就开始铸刀。\n", who);
	write("请这位"+RANK_D->query_respect(who)+"给刀起名字吧！(zhu 刀的名字)\n");
	return 1;
...
int do_zhu(string arg)
{
	...
	if(!me->query_temp("m_blade")) return 0;
	...
	if(!w_name)
		return notify_fail(name() +"说道：使用zhu 刀的名字。\n");
	...
	message_vision("\n只听咯的一声轻响，一柄巨刀从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n" NOR,me );
	say(HIW"\n侍刀见状大叫：神刀初成，人血以祭！！闪开！\n"NOR);
	message_vision(HIW"\n$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。\n"NOR,me);
	message_vision(HIR"\n巨刀透胸穿出，侍刀惨号一声，鲜血溅得你满脸都是！\n"NOR,me);
	message_vision(HIR"\n侍刀往后退开了几步，手捂着伤口，看起来侍刀已经气息粗重了。\n" NOR,me);
	message_vision("\n巨刀又飞了起来，飞到半空，当的一声落回到地上。\n\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
	...
	blade=new("/d/npc/m_weapon/weapon/m_blade",1);
	blade->move(this_player());
	message_vision(CYN"\n侍刀摸起地上，带着斑斑血迹还有些烫手的巨刀，说：刀..已..成...，侍刀的任务....也就完成了....\n"NOR,me );
	say(CYN"\n侍刀艰难的说：刀....您...收好，我该走了....\n"NOR);
	message_vision("\n侍刀刚说完，眉头一皱......\n",me);
	this_object()->die();
	return 1;
...
string blade()
{
	...
	if(me->query("weapon/make") < 0 )  
		return "要铸刀必须找到铸刀的原料。";
	if(me->query("weapon/type") != "刀")
		return "你铸造的兵器好象不是刀吧？";
	if(present("my blade",me))
		return "你的刀好好的，来找我干什么？";
