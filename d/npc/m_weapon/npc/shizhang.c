...
int do_zhu(string arg)
{
	...
	object staff,make_time;
	object me;
	int i;

	me = this_player();

	if(!me->query_temp("m_staff")) return 0;
	...
	sscanf(arg ,"%s" ,w_name);
	if(!w_name)
		return notify_fail(name() +"说道：使用zhu 杖的名字。\n");
	...
	message_vision(HIR"\n巨杖透胸穿出，侍杖惨号一声，鲜血溅得你满脸都是！\n"NOR,me);
	message_vision(HIR"\n侍杖往后退开了几步，手捂着伤口，看起来侍杖已经气息粗重了。\n" NOR,me);
	message_vision("\n巨杖又飞了起来，飞到半空，当的一声落回到地上。\n\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
...
//      me->set("weapon/id",id);
	...
	message_vision(CYN"\n侍杖摸起地上，带着斑斑血迹还有些烫手的巨杖，说：杖..已..成...，侍杖的任务....也就完成了....\n"NOR,me );
	say(CYN"\n侍杖艰难的说：杖....您...收好，我该走了....\n"NOR);
	message_vision("\n侍杖刚说完，眉头一皱......\n",me);
	this_object()->die();
	return 1;
...
string staff()
{
	...
	if(me->query("weapon/make") < 0 )  
		return "要铸杖必须找到铸杖的原料。";
	if(me->query("weapon/type") != "杖")
		return "你铸造的兵器好象不是杖吧？";
	if (present("my staff",me))
		return "你的杖好好的，来找我干什么？";
