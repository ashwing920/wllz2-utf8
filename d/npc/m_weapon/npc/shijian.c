...
{       
	...
	message("vision","侍剑低头检视了一下手中的"+(string)ob->query("name")+"点了点头说：那好，我们现在就开始铸剑。\n", who);
	write("请这位"+RANK_D->query_respect(who)+"给剑起名字吧！(zhu 剑的名字)\n");
	return 1;
}
...
int do_zhu(string arg)
{
	...
	object sword,make_time;
	object me;
	int i;

	me = this_player();
	
	if(!me->query_temp("m_sword")) return 0;
	...
	sscanf(arg ,"%s" ,w_name);
	if(!w_name)
		return notify_fail(name() +"说道：使用zhu 剑的名字。\n");
	...
	message_vision("\n只听咯的一声轻响，一柄巨剑从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n" NOR,me );
	say(HIW"\n侍剑见状大叫：神剑初成，人血以祭！！闪开！\n"NOR);
	message_vision(HIW"\n$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。\n"NOR,me);
	message_vision(HIR"\n巨剑透胸穿出，侍剑惨号一声，鲜血溅得你满脸都是！\n"NOR,me);
	message_vision(HIR"\n侍剑往后退开了几步，手捂着伤口，看起来侍剑已经气息粗重了。\n" NOR,me);
	message_vision("\n巨剑又飞了起来，飞到半空，当的一声落回到地上。\n\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
	...
	sword=new("/d/npc/m_weapon/weapon/m_sword",1);
	sword->move(this_player());
	message_vision(CYN"\n侍剑摸起地上，带着斑斑血迹还有些烫手的巨剑，说：剑..已..成...，侍剑的任务....也就完成了....\n"NOR,me );
	say(CYN"\n侍剑艰难的说：剑....您...收好，我该走了....\n"NOR);
	message_vision("\n侍剑刚说完，眉头一皱......\n",me);
	this_object()->die();
	return 1;
...
string sword()
{
	...
	if(me->query("weapon/type") != "剑")
		return "你铸造的兵器好象不是剑吧？";

	if (present("my sword",me))
		return "你的剑好好的，来找我干什么？";
