...
void create()
{
	set("short","树藤");
	set("long",@LONG
这里长满了树藤，一枝一枝的树藤(teng)遮住了阳光，树藤盘根错节
，你只能在树藤上爬上爬下。
LONG);
	set("item_desc",([
...
int do_insert(string arg)
{
	...
	if ( !living(this_player()) || arg != "shuteng" ) return 0;
	if (me->query("family/family_name")!="慕容世家") return 0;
	skill = (int)me->query_skill("finger", 1);
	if ( skill >101 )
		return notify_fail("你用手插进树藤，但是已经无法提高你的基本指法了！\n");
	if(( skill * skill * skill / 10) > me->query("combat_exp"))
		return notify_fail("也许是实战经验不足，你总是无法熟练基本指法的技巧。\n");
