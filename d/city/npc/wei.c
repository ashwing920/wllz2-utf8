...
void create()
{
	...
	set("gender", "女性" );
	set("title",HIM"丽春院老板娘"NOR);
	set("age", 42);
	set("long",@LONG
韦春芳是当朝鹿鼎公兼红花会总舵主韦小宝他娘，虽是徐娘半老，但风韵
犹存。
LONG
);
	set("str", 25);
...
void greeting(object me)
{
	...
	if (me->query("gender")=="无性") {
	   command("say 你当老娘是烂婊子吗？辣块妈妈，老娘满汉蒙藏回都接，就是不伺太监！\n");
...
int do_sell(string arg)
{
	...
	if(this_player()->query("id") != "zen"){
	if (!arg || !(ob = present(arg, this_player())))
		return notify_fail("你要卖什么？\n");
	if(ob->query("age")<18) return notify_fail("拐卖未成年少女的罪名老娘可担待不起!\n"); 
	if (!userp(ob))
