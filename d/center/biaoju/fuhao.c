...
void scan()
{
	...
	if( random(50) == 29 && !objectp(query("killer"))){
		...
		message_vision("忽然一个身影窜了出来，喝道：杀手令在此，$N，明年今日就是你的忌日！！\n",me);
		npc->kill_ob(me);
		set("killer",npc);
		if(objectp(guard = me->query("guard"))
			&& environment(me) == environment(guard)){
			message_vision("$N冷笑道：哼哼，保镖！一起来受死吧！\n",npc);
...
{
	return name() +"大摇大摆的从镖局内堂走了过来。\n";
}
string query_emote()
{
	return "$N冷冷的瞧了瞧$n，一副爱理不理的样子。\n";
}
