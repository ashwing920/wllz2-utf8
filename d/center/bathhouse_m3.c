...
void create()
{
	set("short", "走廊");
	set("long", @LONG
这里是给到这儿的男人们更衣入浴的，地上是五光十色的彩带和香味
扑鼻的花瓣，昂贵的衣裳满地都是，透过蒙蒙水雾，你可以看到健壮的身
躯....
LONG
);
	set("exits", ([
		"east" : __DIR__"bathhouse_m2",
		"west" : __DIR__"bathhouse_mpool",
	]));
	set("objects", ([
		__DIR__"npc/bellgirl": 1,
	]) );
	setup();
...
int valid_leave(object me, string dir)
{
	...
	int i;
	int withtowel=0;
	if( dir == "west" && ob=present("little bellgirl", this_object()))
	{
	...
	if( dir == "east" && ob=present("little bellgirl", this_object()))
	{
		inv = all_inventory(me);
		for(i=0; i<sizeof(inv); i++)
		{
