...
void create()
{
	set("short", "议事大厅");
	set("long", @LONG
这里是唐门的议事大厅。平时一般不使用，只有在唐门发生重大事情
的时候，唐门的长辈才在这儿聚集商量。整个大厅建筑得很奇怪，四周有
八根朱红大柱地面黝黑发亮，是唐门平时冶炼暗器所剩的铁屑溶制成的整
块。正中是一个很大的八卦图(bagua)。
LONG
);
...
int do_break(string arg)
{
	...
	message_vision("$N走到八卦前，腾身而起，运起全身内力，双掌自上而下重重击向八卦。\n", this_player());


	n = (int)me->query("neili");
	if (n >= 1500){
	message_vision(HIR"$N只听轰得一声巨响！！！八卦竟然翻转，面前出现一个洞。\n"NOR, this_player());
	...
	{
		message_vision(HIW"结果只听一声闷哼，$N被八卦的反弹力震得眼前一黑....\n"NOR, this_player());
