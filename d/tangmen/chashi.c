...
void create()
{
	set("short", "茶室");
	set("long", @LONG
这是唐门待客的茶室。满屋茶香、果香四溢，茶、果都是唐门“百果
山庄”自产的精品。室内摆设很雅致，有几张精致的檀香木桌(table),以
及几把竹椅(chair)四面是檀香雕花窗(window),可以望见窗外的花园。
LONG
);
...
int do_tap(string arg)
{
	...
	message_vision("唐晶不耐烦地对$N说道：刚给你上过茶你接着就要，你不累我还觉得累呢！\n", me);
	return notify_fail("");
	}
...
int valid_leave(object me, string dir)
{
	...
	{
	case 0: 
		return notify_fail("唐晶对着$N用手指刮着自己的小脸蛋：吃饱喝足还要拿，羞！羞！羞！\n");
		break;
	case 1:
		return notify_fail("唐晶嘻嘻笑道：老太太说过，饮食不得带出茶房。\n");
		break;
	}
