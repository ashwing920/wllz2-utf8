...
int valid_leave(object me, string dir)
{
	...
	if(objectp(present("xu tong", environment(me))))
		...
		"女流擅入。" + RANK_D->query_respect(me) +"请下山去吧，免得自讨没趣。\n");
	else if(objectp(present("xu ming", environment(me))))
		return notify_fail("虚明迈步挡在你身前，双手合什说道：阿弥陀佛，"
		"这位" + RANK_D->query_respect(me) + "请留步，恕小寺不接待女客。女施主倘若一心向佛，何妨去峨眉山随喜？\n");
	}
	
	if((!mapp(myfam =me->query("family")) || myfam["family_name"] != "少林派" ) && dir == "eastup") {
	inv = all_inventory(me);
	for(i=sizeof(inv)-1; i>=0; i--)
		if(inv[i]->query("weapon_prop") && (inv[i]->query("equipped")))
		if(objectp(present("xu tong", environment(me))))
			return notify_fail("虚通拦住你说道：这位" +RANK_D->query_respect(me) + "请放下兵刃。少林千年的规矩，外客不得持兵刃上山。\n");
		else if(objectp(present("xu ming", environment(me))))
