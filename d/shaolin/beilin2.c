...
void create()
{
	set("short", "西碑林");
	set("long", @LONG
这里是少林寺的一大片碑林。虽说是碑林，主要是少林历代高僧的佛
塔灵藏(tower)。 一座座佛塔约有二三人高，有汉白玉的，花冈石的，精
工雕凿，薄薄的浮雕面上飞禽走兽，佛谒禅诗，诸佛众生，可谓无所不包
。底座有须弥座的，也有莲花座的，各态纷呈。佛塔间疏疏落落的立着几
块碑(stone)，有的题着几句诗。
LONG
);
	set("exits", ([
...
void check_trigger()
{
	...
	&&	!query("exits/down") ) {
		message("vision", "佛塔忽然发出轧轧的声音，向一侧缓缓移开，向下露出一个黑洞洞的入口。\n", this_object() );
...
void close_passage()
{
	...
	message("vision", "佛塔忽然发出轧轧的声音，缓缓地移回原处，将向下的洞口盖住了。\n", this_object() );
	if( room = find_object(__DIR__"beilin3") ) {
		room->delete("exits/up");
		message("vision", "头顶上的石板忽然发出轧轧的声音，墙上的石块又缓缓地收了回去，阶梯消失了。\n", room );
	}
	delete("exits/down");
}
