...
int do_fish(string arg)
{
        ...
        if(me->query_temp("fishing"))
                return notify_fail("钓鱼要耐心一点嘛！\n");
        if(me->is_busy())
                return notify_fail("你正忙着呢！\n");
        if(me->is_fighting())
                return notify_fail("边打架边钓鱼？不行！\n");
        if(me->query("qi") < 20|| me->query("jing") < 20)
                return notify_fail("身体要紧，还是赶紧休息吧！\n");
        message_vision("$N拿着一把钓鱼杆坐在大石头上钓起鱼来......\n",me);
        me->set_temp("fishing", 1);
        call_out("compelete_fish",8,me);
        return 1;
}

private void compelete_fish(object me)
{
        object ob;
        string badstr;
...
{
        if ( me->query_temp("fishing") )
        	return notify_fail("你还在钓鱼呢，等钓完了再走吧！\n" );
