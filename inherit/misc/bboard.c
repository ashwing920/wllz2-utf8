...
string short()
{
	...
	if( me ) {
		last_read_time = (int)me->query("board_last_read/" + (string)query("board_id"));
...
int do_post(string arg)
{
	...
	if(!wizardp(me) && (int)me->query_skill("literate",1) < 10)
		return notify_fail("你还是个文盲，先去学点「读书识字」吧！\n");

	if(!wizardp(me) && (int)me->query("mud_age") < 86400)
		return notify_fail("小孩子家，留言板不是你涂鸦的地方！\n");

	if(query("wiz_only") && !wizardp(me))
		return notify_fail("这个留言板只有巫师才能够发布信息。\n");

	note = allocate_mapping(5);
	note["title"] = arg;
	note["author"] = me->name(2);
	note["author_id"] = me->query("id");
	note["time"] = time();
	(me->query("ink") ? note["author_discribe"] = me->query("ink"): "");
...
int do_discard(string arg)
{
	...
	if( !arrayp(notes) || num < 1 || num > sizeof(notes) )
		return notify_fail("没有这张留言。\n");
	num--;
	if( notes[num]["author_id"] != (string)me->query("id")
