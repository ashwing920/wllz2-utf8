...
int accept_fight(object ob)
{
	...
	if((string)npc->query("winner") == (string)ob->query("id"))
		return notify_fail(me->name()+"摇了摇头，说道：你已经是武林盟主，还要抢当罚恶使？！\n");

	npc = find_living("shangshan shizhe");
	if(!npc) npc = load_object("/clone/npc/shang-shan");
	if((string)npc->query("winner") == (string)ob->query("id"))
		return notify_fail(me->name()+"摇了摇头，说道：你已经是赏善使者还要抢当罚恶使者？！\n");

	if((string)me->query("winner") == (string)ob->query("id"))
		return notify_fail(me->name()+"摇了摇头，说道：莫非你是走火入魔，连自己都不认得吗？\n");
	...
	if(objectp(weapon = ob->query_temp("weapon")))
		return notify_fail(me->name()+"摇了摇头，说道：挑战罚恶使必须空手！难道你想杀人成！？\n");

	if((int)ob->query("combat_exp") < (int)me->query("combat_exp"))
		return notify_fail(me->name()+"摇了摇头，说道：就凭你这微末得道行就想当罚恶使者？\n");
...
int do_copy(object me, object ob)
{
	...
	me->add("generation",1);
	me->set_name(ob->query("name"),({"fae shizhe","shizhe","fae"}));
	me->set("title","第" + chinese_number(me->query("generation")) + "代罚恶使者");
	ob->set("title", "第" + chinese_number(me->query("generation")) + "代罚恶使者");
	do_clone(me,ob);
	return 1;
}
