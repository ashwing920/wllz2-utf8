...
void init()
{
	...
	if( !pointerp(mails) || !sizeof(mails) ) return;
		write(HIW"\n系统提示：你的信箱中现在共有 " + HIR+CHINESE_D->chinese_number(sizeof(mails)) + HIW" 封信件，请到昆明驿站接收。\n\n"NOR);
	}
}
...
int do_mail(string arg)
{
	...
	if( this_player()->query_skill("literate",1) < 10)
		return notify_fail("你是文盲，先学点「读书识字」吧！\n");
	if( this_player()->query("mud_age") < 26400)
		return notify_fail("小孩子家，别往人家信箱乱塞东西。\n");
...
int do_from()
{
	...
	write(HIY"您的信箱中现在共有 " + sizeof(mails) + " 封信件：\n\n"NOR);
	for(i=0; i<sizeof(mails); i++)
		write(sprintf("[%2d] %-40s From：%-20s\n",
